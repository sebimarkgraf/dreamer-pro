...
class Agent(common.Module):
  ...
  def __init__(self, config, logger, actspce, step, dataset):
    ...
    with tf.device('cpu:0'):
      self.step = tf.Variable(int(self._counter), dtype=tf.int64)
  ...
  def policy(self, obs, state=None, mode='train'):
    ...
    latent, action = state
    embed = self.wm.encoder(self.wm.preprocess(obs, aug=False))
...
class WorldModel(common.Module):
  ...
  def __init__(self, step, config):
    ...
    ################################################################################
    self._ema_encoder = common.ConvEncoder(**config.encoder)
    self._ema_obs_proj = tfkl.Dense(self.rssm._proto, None)
    self._updates = tf.Variable(0, dtype=tf.int64)
    ################################################################################
  ...
  def train(self, data, state=None):
    ...
    modules = [
        self.encoder,
        self.rssm._cell,
        *self.rssm._modules.values(),
        *self.heads.values(),
    ]
    varibs = [self.rssm._prototypes]
    model_grads, model_varibs, model_metrics = self.model_opt.get_grad(
        model_tape, model_loss, modules, varibs)
    if self._updates < self.config.freeze_prototypes_iters:
      model_grads[0] = tf.zeros_like(model_grads[0])
    metrics.update(self.model_opt(model_grads, model_varibs, model_metrics))
    self.ema_update()
  ...
  def loss(self, data, state=None):
    ################################################################################
    data = self.preprocess(data, aug=True)
    embed = self.encoder(data)
    ema_proj = tf.stop_gradient(self.ema_proj(data))
    post, prior = self.rssm.observe(embed, data['action'], state)
    losses = self.rssm.proto_loss(post, embed, ema_proj)
    ################################################################################
    ...
    return model_loss, post, outputs, metrics
  ...
  def preprocess(self, obs, aug=False):
    ...
    # data augmentation
    if aug:
      obs = {k: tf.concat([v, v], axis=0) for k, v in obs.items()}
      obs['image'] = common.random_translate(
          obs['image'],
          self.config.aug.max_delta,
          self.config.aug.same_across_time,
          self.config.aug.bilinear)
    ################################################################################
    return obs
  ...
  def ema_update(self):
    if self._updates % self.config.ema_update_every == 0:
      mix = 1.0 if self._updates == 0 else float(
          self.config.ema_update_fraction)
      for s, d in zip(self.encoder.variables, self._ema_encoder.variables):
        d.assign(mix * s + (1 - mix) * d)
      for s, d in zip(self.rssm._modules['obs_proj'].variables, self._ema_obs_proj.variables):
        d.assign(mix * s + (1 - mix) * d)
  ################################################################################
...
class ActorCritic(common.Module):
  ...
  def train(self, world_model, start, reward_fn):
    ...
    metrics.update(self.actor_opt(
        *self.actor_opt.get_grad(actor_tape, actor_loss, self.actor)))
    metrics.update(self.critic_opt(
        *self.critic_opt.get_grad(critic_tape, critic_loss, self.critic)))
